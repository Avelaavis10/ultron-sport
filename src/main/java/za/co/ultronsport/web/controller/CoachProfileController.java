...
@RestController
@RequestMapping("/api/v1/coach-profiles")
public class CoachProfileController {
    ...
    public CoachProfileResponse create(@Valid @RequestBody CreateCoachProfileRequest request) {
        return CoachProfileResponse.from(coachProfileService.create(request));
    }

    @GetMapping("/{id}")
    public CoachProfileResponse getById(@PathVariable Long id) {
        return CoachProfileResponse.from(coachProfileService.getById(id));
    }
}
